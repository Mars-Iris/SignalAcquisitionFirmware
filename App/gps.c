  ...
  S1_USE_P16P17();//STC切换串口1到GPS串口
	...
	if(Configuration(USARTx,pcmd[Index]) == 1)//波特率适配成功 
	{
		while(strstr(pcmd[Index],CMD_ConfigEnd) == NULL)//查询到配置完成指令，跳出循环，配置结束
		{
			...
			if (temp == 0)//配置失败
			{
				USART_Sendbuffer(USARTx,CMD_GpsConfFail,sizeof(CMD_GpsConfFail));//发送配置失败指令
				
				USART_SendData(USARTx,Index);//发送错指令索引
...
void GPS_EnterConfiguration(u8 mode)	
{
	...
	xdata u8 gpsmodel = 0 ;//获取配置文件，得到gps型号
	gpsmodel = HX10HZ_MOBILESTATION;////获取配置文件，得到gps型号
